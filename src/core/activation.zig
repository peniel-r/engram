...
// Supports .activations/ directory for persistent indices per NEURONA_OPEN_SPEC.md
...
pub const NeuralActivation = struct {
    ...
    pub fn init(allocator: Allocator, graph: *const Graph, bm25: *const BM25Index, vec: *const VectorIndex) !NeuralActivation {
        var activation = NeuralActivation{
            .allocator = allocator,
            .graph = graph,
            .bm25_index = bm25,
            .vector_index = vec,
        ...
        // Ensure .activations/ directory exists per NEURONA_OPEN_SPEC.md
        const activations_dir = try std.fs.cwd().makeOpenPath(ACTIVATIONS_DIR, .{});
        std.fs.cwd().makeDir(activations_dir) catch |err| {
            std.debug.print("Warning: Could not create .activations/ directory: {}\n", .{err});
        };
    ...
    fn computeStimulus(self: *const NeuralActivation, node_id: []const u8, query: []const u8, query_vec: ?[]const f32) f32 {
        ...
        // Vector similarity
        if (self.vector_weight > 0.0 and query_vec) |qv| {
    ...
    fn propagateSignal(self: *const NeuralActivation, allocator: Allocator, initial_stimuli: *const std.StringHashMap(f32), depth: usize) !std.StringHashMap(f32) {
        ...
        while (current_depth < depth) : (current_depth += 1) {
            ...
            while (act_it.next()) |entry| {
                ...
                for (outgoing) |edge| {
                    const decay_factor = @as(f32, @floatFromInt(edge.weight)) / 100.0;
                    const propagated = activation * decay_factor;
